...
function TaskCard({ task, onView }: { task: Task; onView: (id: string) => void }) {
  ...
  return (
    <article ref={setNodeRef} style={style} className={cn("rounded-md border border-border bg-white p-2 shadow-sm", isDragging && "opacity-40")}>
      <div className="flex items-start gap-1">
        ...
        <button type="button" className="min-w-0 flex-1 text-left" onClick={() => onView(task.id)}><p className="line-clamp-2 text-xs font-medium hover:underline">{task.title}</p><p className="mt-1 truncate text-[11px] text-muted">{customer?.name ?? TASK_TYPE_LABEL[task.type]}</p></button>
      </div>
...
function KanbanColumn({ id, label, tone, tasks, onView }: { id: TaskStatus; label: string; tone: "info" | "success" | "neutral"; tasks: Task[]; onView: (id: string) => void }) {
  ...
  return <section ref={setNodeRef} className={cn("flex min-h-[420px] min-w-[260px] flex-1 flex-col rounded-lg border border-border bg-muted-bg/60", isOver && "border-blue-400 bg-blue-50")}>
    ...
    <div className="flex flex-1 flex-col gap-2 p-2">{tasks.map((task) => <TaskCard key={task.id} task={task} onView={onView} />)}{tasks.length === 0 ? <p className="px-1 py-4 text-center text-xs text-muted">Kéo công việc vào đây</p> : null}</div>
  </section>;
}
